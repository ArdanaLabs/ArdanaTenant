let
  hsm = {
    oleg = "ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIAAf/8teVjvyu8Zs/UUx5+s3yEbG1/fK6z1juMowqPIp oleg@nixos-heavy-machine";
    nick = "ssh-rsa  vandenbroeck@cs-vdb.com";
  };
in
{
  users = {
    groups.yubihsm = {};
    users.hsm = {
      isNormalUser = true; 
      openssh.authorizedKeys.keys = [ hsm.oleg hsm.nick ]; 
      extraGroups = [ "yubihsm" ];
    };
  };
  services.udev.extraRules = ''
    SUBSYSTEMS=="usb", ATTRS{idVendor}=="1050", ATTRS{idProduct}=="0030", TAG+="uaccess", RUN{builtin}+="uaccess"
  '';
}
